...
using System.Text;
    ...
    /// 自己放技能结束,目标状态
    /// </summary>
    public class BattleCastSkillEndTargetStatusBuffTrigger : BaseBuffTrigger
    {
        /// <summary>
        /// 
        /// </summary>
        /// <param name="args">{skill, target(BattleUnitVO)}</param>
        /// <returns></returns>
        protected override bool Trigger(object[] args)
        {
            ...
            if (_triggerParams[6] != -1)
            {
                ...
                foreach (Buff buff in target.GetBuffManager().GetAllBuffs(true))
                {
                    if (buff.BuffCfg.Type == _triggerParams[6])
                    {
                        hasBuff = true;
                        break;
                    }
